...
def databasetomodel(modelversion_id, modelname=False):
    ...
    for reaction, name, reaction_id, equation, subsystem, lower_bound, upper_bound, objective_coefficient in reactions: #get all the reactions, for each one: 
        ...
        query="select a.officialname, a.formula, a.abbreviation, a.compartment, a.charge from metabolite a inner join reactionmetabolite on (a.molecule_id=reactionmetabolite.molecule_id) inner join reaction on (reaction.reaction_id=reactionmetabolite.reaction_id) where (reaction.reaction_id=" + str(reaction_id)  +" ) and (a.modelversion_id=%d)"%modelversion
        ...
        for officialname, formula, reactant, compartment, charge in reactants: #get all the metabolites for one reaction:
            ...
            newvalue.notes={'CHARGE':[charge]} #pm to add charge
    ...
    write_cobra_model_to_sbml_file(cobra_model, 'tosbmloutput.xml')
    cursor.close()
    conn.close()   
    return 1
